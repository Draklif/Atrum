...
public class FirstPersonController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // Gets input and calls jump method
        if (enableFlashlight && Input.GetKeyDown(flashlightKey))
        {
            ToggleFlashlight();
        }
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        GUILayout.Label("Flashlight", new GUIStyle(GUI.skin.label) { alignment = TextAnchor.MiddleLeft, fontStyle = FontStyle.Bold, fontSize = 13 }, GUILayout.ExpandWidth(true));

        fpc.enableFlashlight = EditorGUILayout.ToggleLeft(new GUIContent("Enable Flashlight", "Determines if the player is allowed to toggle the flashlight."), fpc.enableFlashlight);

        GUI.enabled = fpc.enableFlashlight;
        fpc.flashlightKey = (KeyCode)EditorGUILayout.EnumPopup(new GUIContent("Flashlight Key", "Determines what key is used to toggle the flashlight."), fpc.flashlightKey);

        if (fpc.enableFlashlight)
        {
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.PrefixLabel(new GUIContent("Flashlight", "Object to be used as flashlight."));
            fpc.flashlight = (Light)EditorGUILayout.ObjectField(fpc.flashlight, typeof(Light), true);
            EditorGUILayout.EndHorizontal();

            EditorGUILayout.BeginHorizontal();
            fpc.flashlightIntensity = EditorGUILayout.Slider(new GUIContent("Flashlight Intensity", "Determines the intensity of the flashlight."), fpc.flashlightIntensity, 0f, 10f);
            EditorGUILayout.EndHorizontal();

            EditorGUILayout.BeginHorizontal();
            fpc.flashlightRange = EditorGUILayout.Slider(new GUIContent("Flashlight Range", "Determines the range of the flashlight."), fpc.flashlightRange, 0f, 200f);
            EditorGUILayout.EndHorizontal();

            EditorGUILayout.BeginHorizontal();
            fpc.flashlightAngle = EditorGUILayout.Slider(new GUIContent("Flashlight Angle", "Determines the cone angle of the flashlight."), fpc.flashlightAngle, 1f, 179f);
            EditorGUILayout.EndHorizontal();
        }
